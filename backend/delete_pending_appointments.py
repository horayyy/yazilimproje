#!/usr/bin/env python
"""
Script to delete pending appointments
"""
import os
import sys
import django

# Setup Django
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'hospital_project.settings')
django.setup()

from core.models import Appointment

def delete_pending_appointments():
    """Delete all pending appointments"""
    pending = Appointment.objects.filter(status='pending')
    count = pending.count()
    
    if count == 0:
        print("✅ Beklemede randevu bulunamadı.")
        return
    
    print(f"⚠️  {count} adet beklemede randevu bulundu:")
    for apt in pending:
        print(f"  - ID: {apt.id}, Hasta: {apt.patient.username}, Tarih: {apt.date}, Saat: {apt.time}")
    
    # Sil
    pending.delete()
    print(f"\n✅ {count} adet beklemede randevu silindi!")

if __name__ == '__main__':
    delete_pending_appointments()

